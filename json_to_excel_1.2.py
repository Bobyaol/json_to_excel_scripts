...
# 两个三级sheet: release_trend中的commits信息, repos中的datas信息
# 这是对json_to_excel.py文件的1.1更新版本，主要是解决了部分二级字段表头和表数据对应不一致的问题
# 这是对json_to_excel.py文件的1.2更新版本，主要是解决了commits页面表头固定的问题
...
#3_1##############################################################################################################
    ...
    origin_second_half_dict_3_1 = total_content[key_2_2]
    ...
    final_second_half_dict_3_1 = []
    for i_3_1 in origin_second_half_dict_3_1:
        single_i = {}
        single_i['date'] = i_3_1['date']
        single_i['phase'] = i_3_1['phase']
        if 'position' in i_3_1.keys():
            single_i['position'] = i_3_1['position']
        else:
            single_i['position'] = 0

        if 'commit_num' in i_3_1.keys():
            single_i['commit_num'] = i_3_1['commit_num']
        else:
            single_i['commit_num'] = 0

        if 'dev_equivalent' in i_3_1.keys():
            single_i['dev_equivalent'] = i_3_1['dev_equivalent']
        else:
            single_i['dev_equivalent'] = 0

        if 'loc' in i_3_1.keys():
            single_i['loc'] = i_3_1['loc']
        else:
            single_i['loc'] = 0

        if "commit_num_trend" in i_3_1.keys():
            single_i['commit_num_trend'] = i_3_1['commit_num_trend']
        else:
            single_i['commit_num_trend'] = 0

        if "dev_equivalent_trend" in i_3_1.keys():
            single_i['dev_equivalent_trend'] = i_3_1['dev_equivalent_trend']
        else:
            single_i['dev_equivalent_trend'] = 0

        if "loc_trend" in i_3_1.keys():
            single_i['loc_trend'] = i_3_1['loc_trend']
        else:
            single_i['loc_trend'] = 0

        if "jira_issue" in i_3_1.keys():
            single_i['jira_issue'] = i_3_1['jira_issue']
        else:
            single_i['jira_issue'] = 0

        if "jira_issue_trend" in i_3_1.keys():
            single_i['jira_issue_trend'] = i_3_1['jira_issue_trend']
        else:
            single_i['jira_issue_trend'] = 0

        if "developers" in i_3_1.keys():
            single_i['developers'] = i_3_1['developers']
        else:
            single_i['developers'] = []

        if "developer_num" in i_3_1.keys():
            single_i['developer_num'] = i_3_1['developer_num']
        else:
            single_i['developer_num'] = 0

        if "dev_equivalent_per_perople" in i_3_1.keys():
            single_i['dev_equivalent_per_perople'] = i_3_1['dev_equivalent_per_perople']
        else:
            single_i['dev_equivalent_per_perople'] = 0

        if "commits" in i_3_1.keys():
            single_i['commits'] = i_3_1['commits']
        else:
            single_i['commits'] = []

        final_second_half_dict_3_1.append(single_i)

    dict_for_key_3_1 = {}
    index_3_1 = 0
    for i in final_second_half_dict_3_1:
